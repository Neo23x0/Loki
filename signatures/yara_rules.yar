...
rule SNOWGLOBE_Babar_Malware {
	...
	strings:
		$mz = { 4d 5a }
		$x0 = "admin\\Desktop\\Babar64\\Babar64\\obj\\DllWrapper" ascii fullword
		...
		$x3 = "System32" fullword ascii
		$x4 = "CONOUT$" fullword ascii
		$x5 = "cmd.exe" fullword ascii
		$x6 = "DLLPATH" fullword ascii
	condition:
		( $mz at 0 ) and filesize < 900000 and filesize > 400000 and
		( 
			$x0 or
			( 3 of ($s*) and 4 of ($x*) ) 
		)
...
rule VisualDiscovery_Lonovo_Superfish_SSL_Hijack {
	...
	condition:
		( $mz at 0 ) and filesize > 1MB and filesize < 2MB and all of ($s*)
}
